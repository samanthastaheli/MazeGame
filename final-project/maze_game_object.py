...
# player class
class Player:
    # variables needed: img, posx, posY
    def __init__(plr, posX, posY):
        # plr = short for player
        plr.posX = posX
        plr.posY = posY
        pygame.image.load(playerImg)
    def build_plr(plr):
        screen.blit(playerImg, (plr.posX, plr.posY))
...
def celebrate():
    # display You Did It! when player is in (x,y) coordinates
    # stop timer
    # display time
    pass

def timer():
    # start timer when player hits first key
    # stop timer when player crosses finish line
    pass
...
def draw_grid(screen, color):
    ...
    #     pygame.draw.rect(screen, color, (i, 0, 2, screen_height))
    #     i += 50
        # screen.create_text(X + 20, i + 10, text=f'{i}')

    # for i in range(left, right, grid_spacing):
    #     screen.create_line(i, top, i, bottom)
    #     screen.create_text(i, top + 10, text=f'{i}')
